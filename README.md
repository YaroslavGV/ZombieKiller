...
### UnitSkin
Скин так-же имеет модули, занимающиеся анимациями движения, мерти, флипа по X. Скин человека имеет уникальный модуль для IK рук. Так-же скин имеет свой набор точек, как например точка для индикатора здоровья.

### UnitProfile
![Profile](https://i.imgur.com/4Rapjmn.png)

В профиле содержится информация о статах, скине, экиперовке, патронов и другом.
Имеется две реализации, `UnitProfile` и `PlayerProfile`, в котором экиперовка задается не из поля профиля и патроны берутя из инвенторя.
